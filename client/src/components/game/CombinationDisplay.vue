<template>
  ...
  <div v-else-if="showEmpty" class="text-center text-sm text-muted-foreground py-2">
...
function getComboClasses(combo: Combination, i: number): string {
  const base = props.selectable ? 'cursor-pointer hover:scale-[1.01] active:scale-[0.99]' : ''
