...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('dice_rolls', (table) => {
    ...
    table.uuid('battle_id').references('id').inTable('battles').onDelete('CASCADE');
    ...
    table.enu('outcome', ['crit_success', 'success', 'fail', 'crit_fail'], {
      useNative: true,
      enumName: 'dice_roll_outcome',
      existingType: true,
    }).notNullable();
    table.enu('context', ['capture_attempt', 'combat', 'penalty_roll', 'event_trigger', 'initial_roll'], {
      useNative: true,
